...
PYBIND11_MODULE(storable, MODULE) {
  ...
  Storable.def(py::pickle(
     [](storable const &x) {
       auto const & buf = h5::serialize(x);
       auto * char_ptr = reinterpret_cast<const char *>(buf.data());
       PyObject * pybuf = PyBytes_FromStringAndSize(char_ptr, buf.size());
     ...
     [](py::tuple t) -> storable {
       ...
       auto size = PyBytes_Size(t[0].ptr());
       auto * buffer = reinterpret_cast<std::byte *>(PyBytes_AsString(t[0].ptr()));
       return h5::deserialize<storable>({buffer, buffer + size});
     }));
